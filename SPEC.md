...
Status: Draft v1 + `symphony-rs` Repository Addendum  
Canonical upstream source: <https://raw.githubusercontent.com/openai/symphony/refs/heads/main/SPEC.md>

## Repository Addendum (Applies to This Repo)

This repository adopts the upstream Symphony specification in full and adds repository-level
architecture and quality constraints. These constraints are language-neutral by intent and must be
preserved by all ports and refactors.

### A. Architecture Constraints

1. Functional core / imperative shell
- Keep orchestration decisions, eligibility rules, backoff math, sorting, and state transitions in
  pure/domain functions.
- Keep side effects (filesystem, process execution, network, clocks, timers, HTTP listeners,
  watchers) in boundary adapters.

2. Effect pattern for all IO
- Model IO explicitly at boundary adapters and keep it replaceable for tests.
- Avoid embedding transport/process/filesystem assumptions inside core policy logic.

3. Parse-don't-validate
- Parse external input directly into semantic domain types where possible.
- Reject invalid payloads at parse boundaries rather than carrying ambiguous primitive values.

4. Semantic domain modeling
- Prefer named domain types and semantic wrappers over unstructured primitive maps in internal
  logic.
- Primitive/structural values are allowed at outer IO boundaries only (wire formats, CLI args,
  storage payloads, network payloads).

5. Private-by-default API surface
- Keep modules, functions, structs, and fields non-public by default.
- Increase visibility only when required by the compiler and cross-module/API boundaries.

### B. Runtime and Product Constraints

1. Framework stack
- The service runtime uses asynchronous orchestration and an HTTP extension.
- The dashboard surface uses SSR with hydration semantics.

2. Workflow contract ownership
- `WORKFLOW.md` remains the runtime contract and policy source for orchestration behavior.

3. Observability requirement
- Structured logs are mandatory.
- Runtime state must be inspectable via machine-readable API endpoints.

### C. Quality and Enforcement Constraints

1. Warning-free policy
- Treat compiler warnings, lints, and static-analysis warnings as errors.

2. Strict lint baseline
- Use strict lint rule sets at the package/module root and keep them at the strongest practical
  level.
- If third-party code generation or tooling injects incompatible relaxations, downgrade only the
  specific affected rules from `forbid` to `deny`, document why, and keep all others strict.
- Keep dependency-duplication checks enforced at `deny`; if transitive ecosystem duplication is
  unavoidable, manage explicit exceptions through linter configuration rather than broad in-code
  `allow` attributes.
- Pedantic/experimental lint sets remain optional.

3. Dependency management discipline
- Add/remove/update dependencies through ecosystem CLI tooling (for example, package-manager add
  commands), not ad-hoc manifest editing.

4. Acceptance testing for user-facing features
- Every user-facing feature must be covered by automated acceptance tests through browser-driven
  flows.

5. Mutation and property testing
- Mutation testing is required and targeted at full kill-rate for the repository’s mutation profile.
- Custom domain validation/parsing logic must be covered by property-based tests.

## SPEC Maintenance Contract (Required Process)

This repository requires spec synchronization as part of normal development.

### 1. When to update `SPEC.md`

Update `SPEC.md` whenever any of the following change:
- Architecture principles
- Runtime behavior, policies, or workflow contract semantics
- Safety/trust posture
- Public observability/API behavior
- Testing or quality gates

### 2. How to update

1. Re-fetch the canonical upstream spec.
2. Re-apply the repository addendum in this file.
3. Ensure all repository-specific constraints remain accurate.
4. Update any in-repo docs that derive from or reference spec behavior (`AGENTS.md`, docs/, and
   workflow-related docs).
5. Commit code and docs together.

### 3. Merge policy

- PRs that modify behavior without corresponding doc/spec updates are non-conformant.
- Pre-commit hooks enforce documentation synchronization checks.

---

## Upstream Specification (Verbatim Mirror)

The following section mirrors the upstream OpenAI Symphony spec for portability and auditability.
